...
REFUGE_RENDERER_VERSION: Final[int] = 1
...
def _draw_sky_details(
    ...
    if context.daypart == "night":
        stars = (
            (88, 72), (154, 116), (244, 64), (318, 138), (406, 84),
            (508, 126), (598, 62), (704, 104), (792, 58), (884, 132),
            (970, 80), (1064, 116), (1168, 62), (1224, 150),
        )
        for index, (x, y) in enumerate(stars):
            radius = 2 if index % 3 else 3
            draw.ellipse((x - radius, y - radius, x + radius, y + radius), fill=(226, 231, 220))
        draw.ellipse((1060, 70, 1128, 138), fill=(225, 221, 188))
        draw.ellipse((1084, 58, 1140, 122), fill=(18, 31, 58))
    elif context.daypart == "morning":
        draw.ellipse((1010, 80, 1082, 152), fill=(244, 211, 142))
    elif context.daypart == "sunset":
        draw.ellipse((1018, 142, 1098, 222), fill=(239, 181, 108))
    else:
        draw.ellipse((1040, 74, 1118, 152), fill=(241, 225, 154))
...
def _draw_mountains(
    ...
    draw.polygon(
        ((424, 288), (612, 176), (736, 270), (902, 144), (1088, 286), (1280, 204), (1280, 300), (424, 300)),
        fill=front,
    )
    if context.season == "winter":
        snow = (221, 225, 218)
        draw.polygon(((138, 192), (170, 170), (199, 196), (177, 189), (164, 202)), fill=snow)
        draw.polygon(((872, 169), (902, 144), (934, 174), (910, 166), (896, 184)), fill=snow)
...
def _draw_ground_details(
    ...
    rocks = ((214, 476, 14), (1042, 470, 18), (312, 616, 12), (978, 626, 11))
    for x, y, radius in rocks:
        draw.ellipse((x - radius, y - radius // 2, x + radius, y + radius // 2), fill=palette["rock"])

    if context.season == "spring":
        flowers = ((262, 536), (334, 558), (952, 548), (1018, 572), (528, 616), (756, 608))
        for index, (x, y) in enumerate(flowers):
            color = (222, 178, 193) if index % 2 else (230, 214, 128)
            draw.ellipse((x - 3, y - 3, x + 3, y + 3), fill=color)
    elif context.season == "autumn":
        leaves = ((250, 548), (296, 564), (992, 536), (1032, 566), (456, 642), (812, 630))
        ...
        for x, y, rx, ry in ((420, 526, 62, 14), (862, 562, 70, 16), (614, 650, 96, 12)):
            draw.ellipse((x - rx, y - ry, x + rx, y + ry), fill=(197, 205, 198))
    else:
        grass = ((272, 548), (328, 570), (970, 542), (1026, 582), (470, 634), (802, 626))
        for x, y in grass:
...
def _draw_site_pad(
    ...
    draw.ellipse((x - radius_x, y - radius_y + 5, x + radius_x, y + radius_y + 7), fill=shadow)
    draw.ellipse((x - radius_x, y - radius_y, x + radius_x, y + radius_y), fill=palette["clearing"])
...
class RefugeWorldRenderer:
    ...
    def render_png(
        ...
        *,
        ...
        width, height = REFUGE_CANVAS_SIZE
        ...
        draw.line((112, 458, 195, 506, 272, 716), fill=_mix(palette["water"], (215, 225, 218), 0.28), width=3)
        ...
        trees = (
            (70, 360, 0.66), (144, 382, 0.74), (222, 404, 0.82),
            (1110, 366, 0.70), (1192, 396, 0.78), (1038, 418, 0.82),
            (284, 474, 0.88), (1000, 484, 0.91), (186, 560, 1.00),
            (1084, 574, 1.02), (330, 652, 1.09), (984, 660, 1.10),
        )
